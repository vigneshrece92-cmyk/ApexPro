...
export const InternalDemoTradingPanel: React.FC<InternalDemoTradingPanelProps> = ({
  ...
  useEffect(() => {
    if (prefillSetup) {
      ...
      if (action === "BUY") {
        ...
        setStopLoss(round2(bid + slDist));
        setTakeProfit(round2(bid - tpDist));
      }
    }
  }, [prefillSetup, liveQuote?.symbol, action]);
  ...
  const potentialGain = round2(lotSize * multiplier * tpDistance);
  ...
  const handleAutoSafeLot = () => {
    ...
    setStatusMessage({
      type: "success",
      text: `Standard 1 lot sizing applied (${multiplier} Qty per lot).`,
    });
  };
  ...
  const handleBE = (ticket: number) => {
    ...
    if (res.success) {
      ...
      setStatusMessage({ type: "success", text: res.message });
      sendTelegramNotification(
        `🛡️ <b>Apex Pro Stop Loss Moved to Break-Even</b>\n━━━━━━━━━━━━━━━━━━━━\n<b>Position:</b> #${ticket}\n<b>Status:</b> Risk-Free Trade Secured (+0.50 buffer)`
      );
    } else {
      setStatusMessage({ type: "error", text: res.message });
    }
  ...
  const handleToggleAutoBot = () => {
    ...
    const newLog: AutoBotLog = {
      ...
      message: nextEnabled
        ? `🤖 PAVP Volume Profile Auto-Bot ACTIVATED. Scanning VAH Reversals, VAL Sweeps & POC Magnet bounces (${account.auto_bot.risk_percent}% risk).`
        : "⏸️ PAVP Auto-Bot PAUSED by user.",
    };
  ...
  const totalClosedProfit = account.history.reduce((sum, t) => sum + (t.profit || 0), 0);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-5xl bg-[#0B0E14] border border-terminal-border rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        {/* Header Bar */}
        <div className="flex items-center justify-between p-4 bg-[#070A0F] border-b border-terminal-border">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-white tracking-wide">
                  Apex Pro Virtual Broker
                </h2>
                <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-[#070A0F] border border-terminal-border text-terminal-muted">
                  <span
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            ...
            <div className="p-3 rounded-lg bg-terminal-card border border-terminal-border">
              ...
              >
                ₹{account.equity.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
              </div>
            ...
            <div className="p-3 rounded-lg bg-terminal-card border border-terminal-border">
              ...
              <div className="text-lg sm:text-xl font-mono font-bold text-cyan-300 mt-1">
                ₹{account.margin_free.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
              </div>
              <span className="text-[10px] text-terminal-muted font-mono">
          ...
          {/* Autonomous Volume Profile (PAVP) Auto-Bot Command Center */}
          <div className="p-4 rounded-xl bg-gradient-to-r from-[#0d1424] via-[#09101d] to-[#070b14] border border-blue-500/30 shadow-lg">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-bold text-white">
                      Autonomous PAVP Volume Profile Auto-Bot
                    </h3>
                    <span
                  ...
                  <p className="text-[11px] text-terminal-muted">
                    Pivot-Anchored Volume Profile (VAH / VAL / POC) • ATM Option Auto-Selector • Max 2 positions
                  </p>
                </div>
              </div>
            ...
            {/* Strict Market Hours Closed Alert Banner */}
            ...
            <div className="mt-3 bg-[#05070D] rounded-lg border border-terminal-border/80 p-2.5 font-mono text-[11px]">
              <div className="flex items-center justify-between pb-1.5 mb-1.5 border-b border-terminal-border/50 text-gray-400 text-[10px] flex-wrap gap-2">
                <span className="flex items-center gap-1">
                  ...
                  <span>AUTONOMOUS VOLUME PROFILE AUDIT</span>
                </span>
                <div className="flex items-center gap-2">
          ...
          <div className="p-4 rounded-xl bg-terminal-card border border-terminal-border space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-200 flex items-center gap-1.5">
                ...
                1-Click Manual Execution ({activeSymbol})
              </span>
              <div className="flex items-center gap-2">
                <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-terminal-muted">Lot Size ({multiplier} Qty / Lot):</span>
                  <button
            ...
            {account.auto_bot.market_mode === "STRICT_REAL" && !marketStatus.isOpen && (
              <div className="p-2.5 rounded-lg bg-amber-950/30 border border-amber-500/30 text-amber-300 text-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 font-mono">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>Real Market Hours: Orders blocked ({marketStatus.statusText}).</span>
                </div>
                <button
                  onClick={() => handleToggleMarketMode("24_7_PRACTICE")}
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span>Execute {action === "BUY" ? "BUY CE" : "BUY PE"} {lotSize} Lot ({lotSize * multiplier} Qty) {activeSymbol} @ ₹{currentOpenPrice.toFixed(2)}</span>
              </div>
              ...
              <div>
                {account.open_positions.length === 0 ? (
                  <div className="text-center py-8 text-terminal-muted text-xs border border-dashed border-terminal-border rounded-lg">
                    No active positions currently open. Execute a trade above or turn on the PAVP Auto-Bot.
                  </div>
                ) : (
                        ...
                        {account.history.map((t) => {
                          ...
                          return (
                            <tr key={t.ticket} className="hover:bg-terminal-hover/40">
                              ...
                              <td className="px-3 py-2 font-bold text-white">{t.optionContractName || t.symbol}</td>
                              <td className="px-3 py-2">
                                <span
                                  className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                                    t.type === "BUY"
                              ...
                              <td className="px-3 py-2 text-gray-300 text-[11px]">
                                {curSym}{fmt2(t.price_open)} → {curSym}{fmt2(t.price_close)}
                              </td>
                              ...
                              <td className="px-3 py-2 text-right font-bold">
                                ...
                                >
                                  {(t.profit || 0) >= 0 ? "+" : ""}{curSym}{fmt2(t.profit)}
                                </span>
                              </td>
                            </tr>
                          );
        ...
        <div className="px-5 py-2.5 bg-[#070A0F] border-t border-terminal-border/70 flex items-center justify-between text-[11px] text-terminal-muted">
          <div className="flex items-center gap-2">
            ...
            <span>Indian Options & MCX Virtual Broker • 0ms Internal Latency</span>
          </div>
