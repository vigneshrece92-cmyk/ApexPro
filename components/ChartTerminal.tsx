...
export const ChartTerminal: React.FC<ChartTerminalProps> = ({
  ...
  // Mode: "tradingview" (Official 100% Real-Time Live Feed) or "smart" (Lightweight Canvas)
  const [chartMode, setChartMode] = useState<"tradingview" | "smart">("tradingview");

  // Indicator & SMC Overlays - Clean minimal defaults
  ...
  }, [chartMode, activeSymbol, timeframe, candles.length, showEMA, showSR, showFib, showPD, showOB, showFVG]);
      ...
      <div className="relative flex-1 min-h-[460px] w-full bg-[#0B0E14]">
        {chartMode === "tradingview" ? (
          ...
          <>
            ...
            <div className="absolute top-3 left-3 pointer-events-none flex flex-wrap items-center gap-2.5 text-[11px] font-mono bg-terminal-bg/85 backdrop-blur px-2.5 py-1.5 rounded border border-terminal-border/80">
            ...
            {smc && (
              <div className="absolute bottom-3 left-3 pointer-events-none hidden md:flex items-center gap-3 text-[10px] font-mono bg-terminal-bg/95 backdrop-blur px-3 py-1.5 rounded border border-terminal-border/80 shadow-lg">
                <div className="flex items-center gap-1">
